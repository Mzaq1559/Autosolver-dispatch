...
export default function OwnerDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      setDrivers(fetchedDrivers.map((d: any) => ({
        ...d,
        lat: d.lat || 0,
        lng: d.lng || 0,
      })))
  ...
  useEffect(() => {
    ...
    socket.on('simulation_state', (data) => {
      ...
      setStatsHistory(prev => {
        const newHistory = [...prev, { 
          time, 
          orders_per_minute: data.statistics.orders_per_minute,
          total_orders_processed: data.statistics.total_orders_processed
        }]
    ...
    const fetchSimStatus = async () => {
      try {
        ...
        setSimPaused(status.is_paused)
        setSimSpeed(status.speed_multiplier)
        setSimTime(status.current_time)
      } catch (err) {
    ...
    <div className="box-border flex h-screen min-h-0 w-full max-w-none flex-col overflow-hidden bg-[#0f0f1a] pt-[60px] text-white">
      ...
      <div className="flex min-h-0 w-full min-w-0 flex-1 flex-row">
        <motion.aside
          ...
          className="box-border flex h-full min-h-0 w-[420px] shrink-0 flex-col overflow-y-auto overflow-x-hidden border-r border-white/5 bg-[#1a1a2e]"
        >
